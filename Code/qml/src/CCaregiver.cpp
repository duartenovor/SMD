...
int CCaregiver::insert(CDatabase &db)
{
    ...
    db.insert("Caregiver","NULL,0,NULL,''");
 ...
 */
int CCaregiver::getNumberElements(CDatabase &db)
{
    std::string d_text;

    db.getTotalElements("Caregiver",d_text);
...
/**
 * Function that looks up the number of elements not defined
 * @return integer with number of elements
 */
int CCaregiver::getElementsNDef(CDatabase &db)
{
    std::string d_text;

    db.get("Caregiver","COUNT (*)","pin_code","''",d_text);
...
std::string CCaregiver::getPin(CDatabase &db, int idCaregiver)
{
    ...
    std::cout << "ENTREI GETPIN" << std::endl;
    std::cout  << "d_text" <<  "|" << d_text << "|" << std::endl;
...
{
	std::string d_text;
    std::cout << "----getFingerprint-----" << std::endl;
